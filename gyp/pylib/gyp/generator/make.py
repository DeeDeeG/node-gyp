...
cmd_alink = rm -f $@ && {os.environ['PYTHON']} ./gyp-mac-tool filter-libtool libtool $(GYP_LIBTOOLFLAGS) -static -o $@ $(filter %.o,$^)
...
cmd_mac_tool = {os.environ['PYTHON']} ./gyp-mac-tool $(4) $< "$@"

quiet_cmd_mac_package_framework = PACKAGE FRAMEWORK $@
cmd_mac_package_framework = {os.environ['PYTHON']} ./gyp-mac-tool package-framework "$@" $(4)
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    if flavor == "mac":
        flock_command = f"{os.environ['PYTHON']} ./gyp-mac-tool flock"
        ...
        header_params.update(
            {
                "copy_archive_args": copy_archive_arguments,
                "flock": f"{os.environ['PYTHON']} ./gyp-flock-tool flock",
                "flock_index": 2,
            }
        )
    elif flavor == "freebsd":
