...
function build (gyp, argv, callback) {
  ...
  function doBuild () {
    ...
    if (!win) {
      // Add build-time bin dependency symlinks (e.g. Python) to PATH
      process.env.PATH = `${path.resolve('build', 'tools', 'bin')}:${process.env.PATH}`
      log.verbose('bin symlinks', 'adding bin symlinks (such as Python) to PATH')
      log.verbose('PATH', 'process.env.path PATH is: ' + process.env.PATH)
    }
