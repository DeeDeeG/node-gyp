...
PythonFinder.prototype = {
  ...
  winDefaultLocations: win
    // On non-Windows, skip computing these and use a dummy path (for tests)
    ? getWinDefaultLocations() : ['C:\\Program Files\\Python9999\\python.exe'],
...
function getWinDefaultLocations () {
  // Find the user's "AppData\Local" and "Program Files" folders.
  ...
  if (!foundLocalAppData) {
    addLog(
      'Warning: node-gyp could not locate the "AppData\\Local" directory.')
    addLog(
      'Ensure the "LOCALAPPDATA" or "USERNAME" environment variables are set' +
      ' so node-gyp can check for Python there.')
  }

  // Construct the array of paths to be returned.
  // The python.org Python installer uses these paths by default on Windows.
  for (const majorMinor of ['39', '38', '37', '36']) {
    if (foundLocalAppData) {
      ...
      // Skip the paths under AppData if AppData's location can't be guessed.
      winDefaultLocationsArray.push(
        `${programFiles}\\Python${majorMinor}\\python.exe`,
        `${programFiles}\\Python${majorMinor}-32\\python.exe`,
        `${programFilesX86}\\Python${majorMinor}-32\\python.exe`
  ...
  // Try os.userInfo() to get the username if process.env doesn't have it
...
// Logs a message at verbose level, but also saves it to be displayed later
// at error level if an error occurs. This should help diagnose the problem.
// (This is the real function, PythonFinder.prototype.addLog points to this.)
