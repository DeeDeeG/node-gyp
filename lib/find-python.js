...
PythonFinder.prototype = {
  ...
  win: win,
  pyLauncher: 'py.exe',
  winDefaultLocations: win
  // Assign a fake path on non-Windows OSes, for use in tests only.
    ? getWinDefaultLocations() : ['C:\\Program Files\\Python999\\python.exe'],
...
function getWinDefaultLocations () {
  ...
  // which are the folders the Python.org installer installs under by default.
  ...
  if (!foundLocalAppData) {
    addLog(
      'Warning: node-gyp could not locate the "[Username]\\AppData\\Local" ' +
      'directory for this user.')
    addLog(
      'Ensure at least one of the "LOCALAPPDATA", "USERNAME", or "USER" ' +
      'environment variables are set so node-gyp can check for Python there.'
    )
  }

  // Create an array of default install paths from the Python.org installer
  ...
  // An alternate way to get the username if process.env has been deleted
  function getOsUserInfo () {
    try {
      return require('os').userInfo().username
    } catch (err) {
      addLog('Warning: unable to get username ' +
        'via os.userInfo().username. Proceeding anyway.')
      addLog(err.stack)
    }
  }
}

// A version of PythonFinder.prototype.addLog, but declared at file-level scope.
// (Can be called from anywhere in this file.)
