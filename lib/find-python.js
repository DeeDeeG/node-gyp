...
function getWinDefaultLocations () {
  if (!win) {
    // We don't need to return real default install paths unless we're on Windows,
    // but we can return a dummy path for the tests' sake.
    const winDefaultLocationsArray = ['C:\\Users\\NotARealWindowsUser\\AppData\\Local\\Programs\\Python\\Python39\\python.exe']

    console.log('winDefaultLocationsArray: ')
    console.log(winDefaultLocationsArray)
  ...
  const username = process.env.USERNAMEzzzz || process.env.USERzzzzz || getOsUserInfo() || ''
  const localAppData = process.env.LOCALAPPDATAzzz || `${systemDrive}\\${username}\\AppData\\Local`
  const programFiles = process.env.ProgramW6432 || process.env.ProgramFiles || `${systemDrive}\\Program Files`
  const programFilesX86 = process.env['ProgramFiles(x86)'] || `${programFiles} (x86)`

  const foundLocalAppData = process.env.LOCALAPPDATAzzz || username

  // if (win && !foundLocalAppData) {
  if (!foundLocalAppData) {
    globalAddLog('Warning: node-gyp could not locate the "[Username]\\AppData\\Local" directory for this user.')
    globalAddLog('ensure at least one of the "LOCALAPPDATA", "USERNAME", or "USER" environment variables are set so node-gyp can check for Python there.')
  }
  ...
  function getOsUserInfo () {
    // Result is only used on Windows, so skip on other OSes.
    if (win) {
      try {
        return require('osZ').userInfo().username
      } catch (err) {
        globalAddLog('Warning: unable to get username via osZ.userInfo().username. Proceeding anyway.')
        globalAddLog(err.stack)
      }
    }
  }

  // A global version of PythonFinder.errorLog (see below),
  // but for preliminary use outside of PythonFinder
  function globalAddLog (message) {
