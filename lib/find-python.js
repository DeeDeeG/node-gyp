...
// Declaring this array with file-level scope makes it possible to
// add to the error log from functions outside of PythonFinder as well.
// Note: PythonFinder.errorLog does not create a copy of fileScopedErrorLog,
// it is simply a pointer to fileScopedErrorLog. Changing one changes the other.
const fileScopedErrorLog = []

function PythonFinder (configPython, callback) {
  this.callback = callback
  this.configPython = configPython
  this.errorLog = fileScopedErrorLog
}
...
PythonFinder.prototype = {
  ...
  win: win,
  pyLauncher: 'py.exe',
  winDefaultLocations: getWinDefaultLocations(),
...
function getWinDefaultLocations () {
  if (!win) {
    // We don't need these paths unless we're on Windows,
    // but we can return a dummy path for the tests' sake on other platforms.
    return ['C:\\Users\\WindowsUser\\AppData\\Local\\Programs\\Python\\Python39\\python.exe']
  }

  // Try to find the user's "AppData\Local" and "Program Files" folders
  ...
  if (!foundLocalAppData) {
    fileScopedAddLog('Warning: node-gyp could not locate the "[Username]\\AppData\\Local" directory for this user.')
    fileScopedAddLog('Ensure at least one of the "LOCALAPPDATA", "USERNAME", or "USER" environment variables are set so node-gyp can check for Python there.')
  }

  // Create an array of all the default install paths from the Python.org installer
  const pythonMajorMinors = ['39', '38', '37', '36']
  const winDefaultLocationsArray = []
  if (foundLocalAppData) {
    for (const majorMinor of pythonMajorMinors) {
  ...
  return winDefaultLocationsArray

  // A fallback option to get the username if process.env has been cleared out/deleted
  function getOsUserInfo () {
    try {
      return require('os').userInfo().username
    } catch (err) {
      fileScopedAddLog('Warning: unable to get username via os.userInfo().username. Proceeding anyway.')
      fileScopedAddLog(err.stack)
    }
  }
}

// A version of PythonFinder.addLog with file-level scope, for use outside of PythonFinder
function fileScopedAddLog (message) {
  log.verbose('find Python', message)
  fileScopedErrorLog.push(message)
}
