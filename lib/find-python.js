...
PythonFinder.prototype = {
  ...
  get winDefaultLocations () {
    ...
    const username = process.env.USERNAMEzzz || process.env.USERzzz || getOsUserInfo() || ''
    ...
    if (!foundLocalAppData) {
      this.addLog('Warning: node-gyp could not locate the "[Username]\\AppData\\Local" directory for this user.')
      this.addLog('Ensure at least one of the "LOCALAPPDATA", "USERNAME", or "USER" environment variables are set so node-gyp can check for Python there.')
    }
    ...
    function getOsUserInfo () {
      try {
        ...
        this.addLog('Warning: unable to get username via osZ.userInfo().username. Proceeding anyway.')
        this.addLog(err.stack)
      }
    }
  }.bind(this),
