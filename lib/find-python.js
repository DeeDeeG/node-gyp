...
if (win && !foundLocalAppData) {
  globalAddLog('Warning: node-gyp could not locate the "AppData\\Local" directory for this user. Not looking for Python under that directory.')
  globalAddLog('ensure at least one of the "LOCALAPPDATA", "USERNAME", or "USER" environment variables are set so node-gyp can look for Python there.')
}
...
function getOsUserInfo() {
  try {
    ...
    // We only use this on Windows, so only log on Windows.
    if (win) {
