...
function configure (gyp, argv, callback) {
  ...
  function createPythonSymlink () {
    ...
    fs.unlink(symlinkDestination, function (err) {
      // Don't warn for ENOENT, warn for other errors.
      // ENOENT is expected unless running configure multiple times.
      ...
      fs.symlink(python, symlinkDestination, function (err) {
        if (err) {
          // The symlink is made on a "best-effort" basis to workaround a corner case
          // (old Python on the PATH, newer Python provided via flag or env var, etc.)
          // Don't throw here, simply warn about the failure.
          // Users likely have newer Python on their PATHs, even without the symlink.
