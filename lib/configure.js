...
function configure (gyp, argv, callback) {
  ...
  function createPythonSymlink () {
    ...
    // Assume the symlink exists (and needs to be cleaned up/replaced)
    // unless we explicitly find out it doesn't exist. Even for most lstat errors.
    let symlinkExists = true
    fs.lstat(symlinkDestination, function (err, stats) {
      if (err) {
        if (err.code === 'ENOENT') {
          // ENOENT is expected unless we have already run configure at least once.
          // No need to warn the user about the default case.
          symlinkExists = false
        } else {
          log.warn('python symlink', 'error when checking whether Python symlink already exists. Assuming it does not exist.\n', err)
        }
      }
      if (symlinkExists) {
