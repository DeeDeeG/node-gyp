...
function configure (gyp, argv, callback) {
  ...
  function createBuildDir () {
    ...
    let deepestBuildDirSubdirectory = buildDir
    if (!win) {
      // Designate a subdirectory in the 'build' dir
      // for symlinks to build-time bin dependencies (e.g. python)
      deepestBuildDirSubdirectory = buildDir + '/tools/bin'
    }
    ...
    fs.mkdir(deepestBuildDirSubdirectory, { recursive: true }, function (err, isNew) {
      ...
      if (win) {
        ...
        // Populate the build-time bin dependency symlinks
        createPythonBinSymlink(deepestBuildDirSubdirectory)
        createConfigFile()
      }
    })
  }

  function createPythonBinSymlink (destinationDir) {
    const symlinkDestination = path.join(destinationDir, 'python3')

    log.verbose('python symlink', `creating symlink to "${python}" at "${symlinkDestination}"`)
    log.verbose('python symlink', 'removing existing symlink, if present.')

    fs.unlink(symlinkDestination, function (err) {
      if (err) {
        log.verbose('python symlink', 'An error occurred when attempting to remove the existing symlink. (This usually means the symlink did not exist before trying to remove it). Ignoring and printing as a warning.')
        log.warn('python symlink', err)
      }
      fs.symlink(python, symlinkDestination, function (err) {
        if (err) {
          log.verbose('python symlink', 'Error when creating symlink!')
          return callback(err)
        }
        log.verbose('python symlink', 'created symlink.')
      })
    })
  }
